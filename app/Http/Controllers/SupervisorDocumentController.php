...
class SupervisorDocumentController extends Controller implements HasMiddleware
{
    public static function middleware(): array
    {
        return [
            'auth',
            new Middleware(PermissionMiddleware::using('supervisor-management.view'), ['index', 'preview', 'download']),
            new Middleware(PermissionMiddleware::using('supervisor-management.edit'), ['store', 'destroy']),
        ];
    }

    public function index(Request $request, User $supervisor)
    {
        abort_unless($supervisor->hasRole('Supervisor'), 404);
        $supervisor->load('supervisorProfile.depot');

        $documentTypes = $this->documentTypes();

        if ($request->ajax()) {
            $query = SupervisorDocument::with('documentType')
            ...
            return DataTables::of($query)
                ...
                ->addColumn('action', function ($row) {
                    $previewUrl = route('supervisor-documents.preview', $row->id);
                    $downloadUrl = route('supervisor-documents.download', $row->id);
                    $deleteButton = '';

                    if (auth()->user()->can('supervisor-management.edit')) {
                    ...
                    $viewButton = auth()->user()->can('supervisor-management.view')
        ...
        return view('supervisor-management.documents.index', compact('supervisor', 'documentTypes'));
    }

    public function store(Request $request, User $supervisor)
    {
        abort_unless($supervisor->hasRole('Supervisor'), 404);
        ...
        $path = $file->store('supervisor-documents/' . $supervisor->id, 'public');

        $document = SupervisorDocument::create([
    ...
    public function download(SupervisorDocument $supervisorDocument)
    {
        abort_unless($supervisorDocument->supervisor?->hasRole('Supervisor'), 404);
        abort_unless(Storage::disk('public')->exists($supervisorDocument->file_path), 404);

        return Storage::disk('public')->download(
            $supervisorDocument->file_path,
            $supervisorDocument->original_name ?: basename($supervisorDocument->file_path)
        );
    }

    public function preview(SupervisorDocument $supervisorDocument)
    {
        abort_unless($supervisorDocument->supervisor?->hasRole('Supervisor'), 404);
        abort_unless(Storage::disk('public')->exists($supervisorDocument->file_path), 404);

        return response()->file(Storage::disk('public')->path($supervisorDocument->file_path));
    }

    public function destroy(SupervisorDocument $supervisorDocument)
    {
        abort_unless($supervisorDocument->supervisor?->hasRole('Supervisor'), 404);

        if ($supervisorDocument->file_path) {
            Storage::disk('public')->delete($supervisorDocument->file_path);
        }

        $supervisorDocument->delete();
    ...
    private function documentTypes()
    {
        return HrmsDocumentType::where('is_active', true)
            ->whereIn('applicable_for', ['all', 'supervisor'])
            ->orderBy('name')
            ->get();
    }
