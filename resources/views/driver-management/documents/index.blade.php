                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="{{ route('driver-management.documents.store', $driver->id) }}" method="POST"
        ...
        <script>
            $(function () {
                ...
                var table = $('#table').DataTable({
                    ...
                    searching: false,
                    ajax: {
                        url: "{{ route('driver-management.documents.index', $driver->id) }}"
                    },
                    columns: [
                ...
                $('#addDocumentModal').on('show.bs.modal', function (event) {
                    if ($(event.relatedTarget).hasClass('add-document-link')) {
                        ...
                        $('#documentForm').attr('action', "{{ route('driver-management.documents.store', $driver->id) }}").data('editing', false);
                ...
                $('#documentForm').on('submit', function (e) {
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            ...
                            form[0].reset();
                            form.attr('action', "{{ route('driver-management.documents.store', $driver->id) }}").data('editing', false);
                            $('#addDocumentLabel').text('Add Document');
                            $('#documentFile').prop('required', true);
            ...
            function deleteDocument(id) {
                deleteRecord('/driver-documents/' + id, 'table', 'Do you really want to delete this document?');
            }
        </script>
    @endsection
</x-app-layout>
