...
<x-app-layout>
    <div class="page-title">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('driver-management.index') }}">Driver Management</a></li>
    ...
    @php
        ...
        $verification = fn ($value) => \App\Models\DriverProfile::VERIFICATION_STATUSES[$value] ?? '-';
    ...
    <a href="{{ route('driver-management.index') }}" class="btn-back-print-cs">
        <i class="fa-solid fa-arrow-left"></i>
        <span>Back</span>
    </a>

    <a href="{{ route('driver-management.download-pdf', $record->id) }}" class="btn-download-print-cs">
        ...
        <span>Download PDF</span>
                        ...
                        <div class="col-lg-9 mb-3">
                            <div class="preview-widget-info li">
                                <h3>{{ $record->name }}</h3>
                                <ul>
                                    <li>Driver Code : <span>{{ $record->code ?: '-' }}</span></li>
                                    ...
                                    <li>License Type : <span>{{ $profile?->license_type_label ?: '-' }}</span></li>
                                    <li>License Expiry : <span>{{ $date($profile?->expiry_date) }}</span></li>
                                    <li>Role : <span>{{ $record->roles->pluck('name')->implode(', ') ?: 'Driver' }}</span></li>
                                    <li>Status :
                                        @if ($record->is_active)
                        ...
                        <div class="col-lg-6 mb-3">
                            <div class="v-preview-widget s-preview-address">
                                ...
                                <ul>
                                    <li><label>License Number :</label> <span>{{ $profile?->license_number ?: '-' }}</span></li>
                                    <li><label>License Type :</label> <span>{{ $profile?->license_type_label ?: '-' }}</span></li>
                                    <li><label>Issue Date :</label> <span>{{ $date($profile?->issue_date) }}</span></li>
                                    <li><label>Expiry Date :</label> <span>{{ $date($profile?->expiry_date) }}</span></li>
                                    <li><label>Badge Number :</label> <span>{{ $profile?->badge_number ?: '-' }}</span></li>
                                    <li><label>Badge Expiry :</label> <span>{{ $date($profile?->badge_expiry_date) }}</span></li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-12 mb-3">
                            <div class="v-document-view">
                                <h6>Documents</h6>
                                <div class="row">
                                    @forelse ($record->driverDocuments as $document)
                                        <div class="col-lg-4 col-md-6 mb-3">
                                            <div class="v-doc-preview s-doc-preview">
                                                ...
                                                <a href="#!" class="mb-3 view-document"
                                                    ...
                                                    data-preview="{{ route('driver-documents.preview', $document->id) }}"
                                                    data-download="{{ route('driver-documents.download', $document->id) }}">
                                                    View
                                                </a>
                                                <small>
